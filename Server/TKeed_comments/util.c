...
int read_conf(char *filename , tk_conf_t *conf){
	...
	while(fgets(curr_pos, buff_len - pos, fp)){
		// 定位每行第一个界定符位置
		dellim_pos = strstr(curr_pos,DELIM);
		if(!delim_pos)
			return TK_CONF_ERROR;
...
int socket_bind_listen(int port){
	...
	// 消除bind时候的"Address already in use"错误
	int optval = 1;
	iof(setsockopt(listen_fd, SOL_SOCKET, SO_REUSERADDR, (const void*)&optval,sizeof(int)) == -1){
		return -1;
	}

	// 设置服务器 IP 和port ，和监听描述符绑定
...
void accept_connection(int listen_fd, int epoll_fd, char *path){
	...
	// 文件描述符可以读，边缘触发， 保证一个socket 连接在任一时刻纸杯一个线程处理
