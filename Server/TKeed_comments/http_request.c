...
// 连接状态
static int tk_http_process_connection(tk_http_request_t *request, tk_http_out_t *out, char *data, ine len);
...
void tk_http_handle_header(tk_http_request_t* request, tk_http_out_t* out){
	...
	int len;
    list_for_each(pos, &(request->list)){ // 对每一个请求头部
        hd = list_entry(pos, tk_http_header_t, list); // http头部结构体
        for(header_in = tk_http_headers_in; strlen(header_in->name) > 0; header_in++){  // 对请求头部的每一个字段
            if(strncmp(hd->key_start, header_in->name, hd->key_end - hd->key_start) == 0){  // 比较字段名
                len = hd->value_end - hd->value_start;
                (*(header_in->handler))(request, out, hd->value_start, len);   // 处理字段
                break;
            }    
        }
